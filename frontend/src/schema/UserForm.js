...
const activities = ['Asistencial', 'Investigacion', 'Docencia', 'Administracion']
...
export const sex = ['Femenino', 'Masculino']
...
export const perfil = ['Estudiante', 'Profesional']
export const level_PBE = [1, 2, 3, 4, 5]


// Returns tipish nacionalities from txt the other txt has 1
export const nacionalities = async function loadNacionalities(nameFile = 'nacionalidad_tipica.txt') {
    ...
    return fetch(dirName)
        .then(response => {
            if (!response.ok) {
                ...
                return toast.error(`Error al recibir respuesta status ${response.status}`)
            }
...
export function loadCCAA(nameFile = 'CCAA.xml') {
    ...
    return fetch(dirName)
        ...
        .then(str => {
            ...
            console.log(`Error en fetch ${err} del fichero ${nameFile} ruta ${dirName}`)
            return toast.error (`Error en fetch ${err} del fichero ${nameFile} ruta ${dirName}`)
        })
}
